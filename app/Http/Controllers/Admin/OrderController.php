...
class OrderController extends Controller
{
    ...
    public function postOrderStatusUpdate(Order $order, Request $request){
        if($request->input('status') == "1" || $request->input('status') == "2" || $order->status == "6" || $order->status == "100"):
            ...
            if($request->input('status') == "100" && is_null($order->reject_at)):
                $order->reject_at = date('Y-m-d h:i:s');
            endif;
            if($order->save()): 
                $user = $order->getUser;     
                ...
                return back()->with('message', 'Estado actualizado con éxito')->with('typealert',"success"); 
            endif;
        endif;
    }
}
